...
import type { GatePassReceipt, Snapshot } from '../types/index.js';

export interface PromoteOptions {
  source: string;
  to: string;
  snapshotId?: string;      // promote a specific snapshot instead of current
  requireGatePass?: string; // path to gate-pass receipt file
  json?: boolean;
  dryRun?: boolean;
}
...
export function runPromote(opts: PromoteOptions): void {
  ...
  if (opts.requireGatePass) {
    ...
    let receipt: GatePassReceipt;
    try {
      receipt = JSON.parse(fs.readFileSync(receiptPath, 'utf-8')) as GatePassReceipt;
    } catch {
...
function outputResult(result: PromoteResult, json?: boolean): void {
  if (json) {
